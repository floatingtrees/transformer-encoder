...
def train_step(compressor, output_model, sequence, batch_size, optimizer, streams): # streams is an array of cuda streams with size batch_size
    ...
    while start <= (length - batch_size):
        ...
        if (torch.sum(torch.isnan(preds)).item() > 0.5):
            print("NAN IN PREDS")
            continue
        if (torch.sum(torch.isnan(labels)).item() > 0.5):
            print("NAN IN LABELS")
            continue
        loss = torch.clamp(torch.nn.functional.cross_entropy(preds, labels), 0, 100)
        loss.backward()
        losses.append(loss.item())
        torch.nn.utils.clip_grad_norm_(compressor.parameters(), 0.2)
        torch.nn.utils.clip_grad_norm_(output_model.parameters(), 0.2)
        optimizer.step()
...
dataloader = DataLoader(dataset, 1, num_workers = 1, shuffle=True)
...
for epoch in range(num_epochs):
    for i, batch in enumerate(dataloader):
        try:
            ...
            #print(time.perf_counter() - start, batch.shape)
            running_losses = running_losses + losses
            if i % 10 == 0:
                print(f"TEXT {i//10}: Loss is {listmean(running_losses)}, most recent loss is {listmean(running_losses[-20:])}")
            if i != 0 and i % 1000 == 0:
                print('SAVING STATE')
